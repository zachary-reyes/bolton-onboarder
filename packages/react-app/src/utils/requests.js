...
export const getApiGnosis = async (networkName, endpoint) => {
  ...
  export const fetchSafeDetails = async (networkName, vault) => {
    try {
      return await getApiGnosis(
        networkName,
        `safes/${utils.getAddress(vault.safeAddress)}`,
      );
    } catch (error) {
      console.error(error);
  ...
  export const fetchSafeIncomingTxs = async (networkName, vault) => {
    try {
      return await getApiGnosis(
        networkName,
        `safes/${utils.getAddress(vault.safeAddress)}/incoming-transfers/`,
      );
    } catch (error) {
      console.error(error);
      // throw new Error(error);
  ...
  export const fetchSafeBalances = async (networkName, vault) => {
    try {
      return await getApiGnosis(
        networkName,
        `safes/${utils.getAddress(vault.safeAddress)}/balances/`,
      );
    } catch (error) {
      console.error(error);
      // throw new Error(error);
